...
use crate::{
    CellCount,
    layout::bg_layout::init_bg_dimensions,
};
...
const BORDER_TO_CELL_FG_RATIO: (f32, f32) = (1., 6.); //1:6
...
impl CellDimensions {
    fn new(nrow: usize, ncol: usize) -> Self {
        let total_size = 1. / usize::max(nrow, ncol) as f32;
        let fg_size = (total_size * BORDER_TO_CELL_FG_RATIO.1)
...
pub fn init_cell_dimensions(
    cell_count: Res<CellCount>,
    mut commands: Commands,
) {
    commands.insert_resource(CellDimensions::new(
        cell_count.nrow,
        cell_count.ncol,
    ));
}
...
fn update_cell_dimensions(
    ...
    for _event in resized_events.read() {
        *cell_dimensions =
            CellDimensions::new(cell_count.nrow, cell_count.ncol);
    }
}
